// This File contains all darwing function
...
void draw_main_ship(int x, int y)
{
    ...
    if(isRotate)
    glRotatef(ship_rotation,0.0,0.0,1.0);
    //Letf Wing
    glColor3f(1, 0, 0);
    glBegin(GL_POLYGON);
        glVertex2f(0,0);
...
{
    long long int temp_score = score;
    int stk[20],sp=0;
    //sp denotes the index of array stk
    while(temp_score>0)
    {
        stk[sp++] = (temp_score%10);
        temp_score /= 10;
    }
    sp--;
    while(sp>=0)
...
void draw_score_board()
{
    ...
    glBegin(GL_POLYGON);
          glVertex2d(0,0);
          glVertex2d(SCORE_BOARD_WIDTH,0);
          glVertex2d(SCORE_BOARD_WIDTH,SCORE_BOARD_HEIGHT);
          glVertex2d(0,SCORE_BOARD_HEIGHT);
    glEnd();
    glLineWidth(5);
    glColor3f(0,0,1);
...
void draw_pause_menu()
{
  glPushMatrix();
        ...
        glBegin(GL_POLYGON);
            ...
            glVertex2f(0,300);
        glEnd();
        glLineWidth(4);glColor3f(0.3,0.7,0.5);
        glBegin(GL_LINE_LOOP);
            glVertex2f(0,0);
            glVertex2f(500,0);
            glVertex2f(500,300);
            glVertex2f(0,300);glEnd();
        glLineWidth(1);
            glPushMatrix();
...
void draw_background()
{
    ...
    if(k==1)
    {
        num_stars = STAR_MIN + rand() %((int)(STAR_MAX-STAR_MIN+1));
        for(int i=0;i<=num_stars;i++)
        {
            star_pos[i][0]=rand()%((int)width);
            star_pos[i][1]= rand()%((int)height);
            star_size[i]= (STAR_MIN_SIZE + rand() % ((int)(STAR_MAX_SIZE-STAR_MIN_SIZE+1)) ) /(100.0);
        }
     }
...
{
    ship_x = (ship_x * new_width)  /width;
    width = (GLdouble) new_width;
    height =(GLdouble) new_height;
    adjust_menu_bounds();
